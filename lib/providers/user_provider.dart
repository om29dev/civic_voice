...
class UserProfile {
  ...
  UserProfile copyWith({
    ...
    return UserProfile(
      ...
      applicationsCount: applications?.length ?? this.applicationsCount,
      completedCount: applications?.where((a) => a.status == ApplicationStatus.approved).length ?? this.completedCount,
      pendingCount: applications?.where((a) => a.status == ApplicationStatus.submitted || a.status == ApplicationStatus.verified).length ?? this.pendingCount,
    ...
    _currentUser = UserProfile.registered(
      ...
      applications: [
        UserApplication(
          ...
          currentStep: 'BDO Verification',
          nextStep: 'Final Approval',
          timeline: [
            ApplicationEvent(title: 'Submitted', description: 'Application filed online', timestamp: DateTime.now().subtract(const Duration(days: 5))),
            ApplicationEvent(title: 'Verified', description: 'Document verification completed', timestamp: DateTime.now().subtract(const Duration(days: 2))),
          ],
        ),
      ],
    );
